...
def main():
    ...
    bp_usage_by_game, bp_quality_by_game = build_bp_usage_by_game(pit_by_date)
    ...
    for d in dates:
        ...
        for g in todays_games:
            ...
            h_prs = shrink(hrs, hg, lrs, TEAM_PRIOR_G)
            h_pra = shrink(hra, hg, lra, TEAM_PRIOR_G)
            h_pwp = shrink(hwp, hg, lwp, TEAM_PRIOR_G)
            a_prs = shrink(ars, ag, lrs, TEAM_PRIOR_G)
            a_pra = shrink(ara, ag, lra, TEAM_PRIOR_G)
            a_pwp = shrink(awp, ag, lwp, TEAM_PRIOR_G)
